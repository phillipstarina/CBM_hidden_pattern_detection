...
─────────────────────────────────────────────
KEY CHANGES in v5.0:
  • Parameter weightage section is DYNAMIC:
    - Populated from dataset's numeric columns after upload
    - Each numeric column gets a weight spinbox (default = equal split)
    - Live total shows when weights sum to 100%
  • Robust file loader:
    - Accepts ANY file (.csv, .xlsx, .xls, .xlsb, .xlsm, .ods, .tsv, .txt, .json, .parquet)
    - Never crashes — shows a clear user-friendly error instead
    - Tries multiple engines automatically
  • Single Export button (Excel multi-sheet)
  • Event detection panel (reads from dataset column)
  • Colour-coded numbered sidebar sections
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# ROBUST FILE LOADER  — never crashes, tries every engine
# ══════════════════════════════════════════════════════════════════════════════


def load_any_file(filepath):
    """
    Load a data file of ANY type into a pandas DataFrame.
    Tries multiple strategies; raises RuntimeError with a clear message if all fail.
    """
    import pandas as pd
    ...
    if ext in ("csv", "tsv", "txt", ""):
        ...
        strategies += [
            ...
            lambda f:           pd.read_csv(
                f, sep=None, engine="python", encoding="latin-1"),
        ]
    ...
    if ext in ("xlsx", "xlsm", "xlsb", "xls", "ods", "odf", "odt"):
        ...
        if ext in ("ods", "odf", "odt"):
            engines += ["odf", None]
        engines += [None]   # pandas default fallback
        for eng in engines:
            if eng:
    ...
    # If extension unknown, try all common ones
    if not strategies:
        strategies += [
    ...
    # All failed — build a clear message
    short_errors = "\n".join(f"  • {e}" for e in errors[:5])
    raise RuntimeError(
        f"Could not read '{os.path.basename(filepath)}'.\n\n"
        f"Attempted {len(strategies)} loading strategies. Errors:\n{short_errors}\n\n"
        f"Supported formats: CSV, TSV, XLSX, XLS, XLSB, XLSM, ODS, JSON, Parquet, Feather, HDF5, Pickle.\n"
        f"Make sure the file is not open in another application and is not password-protected."
    )
...
# Weight widgets keyed by column name (rebuilt on each dataset load)
weight_vars = {}   # {col: StringVar for spinbox}
weight_row_frames = []   # tk.Frame rows in the weight panel
weight_sum_var = None
weight_sum_lbl = None
...
def _mini_bar(parent, label, frac, col, label_width=8):
    ...
    tk.Frame(outer, bg=col, height=7).place(
        relwidth=max(frac, 0.03), relheight=1.0)
...
# ══════════════════════════════════════════════════════════════════════════════
# DYNAMIC WEIGHT PANEL — rebuilt from numeric columns after every dataset load
# ══════════════════════════════════════════════════════════════════════════════
def _update_weight_sum(*_):
    """Recompute and display live total of all weight spinboxes."""
    total = 0.0
    ...
    weight_sum_var.set(
        f"Total: {total:.1f}%  {'✔  OK' if ok else '⚠  should be 100'}")
    weight_sum_lbl.config(fg=SUCCESS2 if ok else WARN)


def rebuild_weight_panel(num_cols):
    """
    Destroy existing weight rows and rebuild from num_cols list.
    Called every time a new dataset is loaded.
    """
    ...
    for i, col in enumerate(num_cols):
        ...
        tk.Label(row, text="%", bg=CARD2, fg=FG_DIM,
...
def get_manual_weights():
    """
    Returns {col: normalised_weight_pct} from spinboxes.
    Normalises so total = 100.  Returns ({}, 0) if nothing set.
    """
    raw = {}
...
# ══════════════════════════════════════════════════════════════════════════════
# MODULE ① — DATA LOADING  (robust, any file)
# ══════════════════════════════════════════════════════════════════════════════
def upload_dataset():
    global raw_data
    f = filedialog.askopenfilename(
        ...
        filetypes=[
            ("All supported files",
             "*.csv *.tsv *.txt *.xlsx *.xls *.xlsb *.xlsm *.ods *.json *.parquet *.feather *.h5 *.hdf5 *.pkl *.pickle"),
            ("CSV / TSV",            "*.csv *.tsv *.txt"),
    ...
    def _load():
        ...
        try:
            df = load_any_file(f)
            # Basic sanity: need at least 1 row and 1 column
            if len(df) == 0:
            ...
            app.after(0, lambda: _post_load(df, f, num_cols))
        except Exception as e:
            app.after(0, lambda: _load_error(str(e)))

    def _post_load(df, filepath, num_cols):
        ...
        # Rebuild weight panel with numeric columns from this dataset
        ...
        progress_stop()
        set_status(
            f"✔  Loaded: {os.path.basename(filepath)}  ({len(df):,} rows × {len(df.columns)} cols)", SUCCESS)
...
def _find_event_column(df):
    keywords = ["status", "state", "event",
                "mode", "condition", "operation", "type"]
    for c in df.columns:
...
def analyse_events(df, anomaly_labels):
    ...
    def _classify(val):
        vl = val.lower()
        if any(k in vl for k in active_kw):
            return "active"
        if any(k in vl for k in inactive_kw):
            return "inactive"
        return "other"
...
def plot_clusters(df, xcols, ycols, hex_colors):
    ...
    y_col = (ycols[0] if ycols else (xcols[1] if len(
        xcols) > 1 else xcols[0] if xcols else None))
...
def plot_pca(X, labels, hex_colors):
    ...
    for cl in sorted(np.unique(labels)):
        mask = labels == cl
        col = hex_colors[int(cl) % len(hex_colors)]
        ax.scatter(Z[mask, 0], Z[mask, 1], color=col, s=45, edgecolors="#ffffff22", linewidths=0.5, zorder=3,
                   label=f"Cluster {cl}  ({int(mask.sum()):,} wells)")
...
def plot_hidden_patterns(X_w, anomaly_labels, xcols, detector_name):
    ...
    if X_w.shape[1] >= 2:
        ...
        ax.text(0.3, 0.5, "No feature data",
                transform=ax.transAxes, color=FG_DIM)
        return fig
    nm = anomaly_labels == 1
    am = anomaly_labels == -1
    ...
    if am.sum() > 0:
        ...
        ax.annotate(
            f"◆ {int(am.sum())} anomal{'y' if am.sum()==1 else 'ies'} detected",
...
def plot_weight_chart(weight_map, method_desc):
    ...
    bars[max_idx].set_edgecolor("#ffffff88")
    ...
    ax.set_xlim(0, max(values_s)*1.22)
    ax.margins(0.03, 0.15)
...
def plot_event_chart(df, event_result):
    ...
    if not event_result["has_events"]:
        ...
        ax.set_title("Operational Event Analysis", color=COL_EVENTS,
                     fontsize=13, pad=12, fontweight="bold")
        return fig

    ec = event_result["event_col"]
    ...
    vs = [v for v, _, _ in sorted_pairs]
    ls = [l for _, l, _ in sorted_pairs]
    cs = [c for _, _, c in sorted_pairs]
    bars = ax1.barh(ls, vs, color=cs, edgecolor="#ffffff22",
    ...
    ax1.set_title(f"Event Distribution\n(column: '{ec}')",
                  color=COL_EVENTS, fontsize=10, pad=8, fontweight="bold")
    ...
    ax2.set_title("Proportion", color=COL_EVENTS,
...
def build_cluster_legend(parent, df, hex_colors):
    for w in parent.winfo_children():
        w.destroy()
    tk.Label(parent, text="CLUSTER LEGEND", bg=CARD, fg=COL_CLUSTER,
    ...
    for cl in clusters:
        ...
        tk.Label(txt, text=f"Cluster {cl}", bg=CARD2,
                 fg=col, font=FONT_H3, anchor="w").pack(anchor="w")
        tk.Label(txt, text=f"{cnt:,} wells  ({pct:.1f}%)", bg=CARD2,
                 fg=FG_MID, font=FONT_XS, anchor="w").pack(anchor="w")
    _divider(parent)
    for cl in clusters:
        cnt = int((df["cluster"] == cl).sum())
...
def build_anomaly_legend(parent, n_normal, n_anomaly):
    for w in parent.winfo_children():
        w.destroy()
    tk.Label(parent, text="PATTERN LEGEND", bg=CARD, fg=ANOMALY_C,
             font=FONT_H3, justify="center").pack(pady=(12, 4), padx=8)
    _divider(parent, ANOMALY_C)
    total = n_normal+n_anomaly
    for label, count, col, icon in [("Normal Wells", n_normal, NORMAL_C, "●"), ("Anomaly Wells", n_anomaly, ANOMALY_C, "◆")]:
        ...
        tk.Label(txt, text=f"{count:,} wells  ({pct:.1f}%)", bg=CARD2,
                 fg=FG_MID, font=FONT_XS, anchor="w").pack(anchor="w")
    if total:
        _divider(parent)
...
def build_weight_legend(parent, weight_map, method_desc):
    ...
    tk.Label(parent, text="PARAM WEIGHTS", bg=CARD, fg=COL_WEIGHTS,
             font=FONT_H3, justify="center").pack(pady=(10, 2), padx=8)
    tk.Label(parent, text="★ = most influential", bg=CARD, fg=FG_DIM,
             font=FONT_XS, justify="center").pack(pady=(0, 4))
    _divider(parent, COL_WEIGHTS)
    if not weight_map:
        tk.Label(parent, text="Run analysis first", bg=CARD,
                 fg=FG_DIM, font=FONT_XS).pack(pady=8)
        return
    ...
    for i, (param, wt) in enumerate(sorted_wm):
        ...
        row.pack(fill="x", padx=8, pady=2)
        tk.Canvas(row, bg=col, width=18, height=18,
                  highlightthickness=0).pack(side="left", padx=(0, 6))
        txt = tk.Frame(row, bg=CARD2 if is_max else CARD)
        txt.pack(side="left", fill="x", expand=True)
        tk.Label(txt, text=f"{'★ ' if is_max else ''}{param[:22]}", bg=CARD2 if is_max else CARD,
                 fg=col, font=FONT_SM, anchor="w").pack(anchor="w")
        tk.Label(txt, text=f"{wt:.1f}%", bg=CARD2 if is_max else CARD,
                 fg=FG_MID, font=FONT_XS, anchor="w").pack(anchor="w")
...
def build_event_legend(parent, event_result):
    ...
    if not event_result["has_events"]:
        tk.Label(parent, text="No event column\nin dataset", bg=CARD,
                 fg=WARN, font=FONT_SM, justify="center").pack(pady=8)
        return
    counts = event_result["event_counts"]
    ...
    for label, cnt in sorted(counts.items(), key=lambda x: -x[1]):
        ...
        txt = tk.Frame(row, bg=CARD2)
        txt.pack(side="left", fill="x", expand=True)
        tk.Label(txt, text=label[:20], bg=CARD2, fg=col,
                 font=FONT_SM, anchor="w").pack(anchor="w")
        tk.Label(txt, text=f"{cnt:,}  ({cnt/total*100:.1f}%)", bg=CARD2,
                 fg=FG_MID, font=FONT_XS, anchor="w").pack(anchor="w")
...
def _fill_toolbar(bar, mpl_canvas, tab_name):
    ...
    nav_frame = tk.Frame(bar, bg="#080e1a")
    nav_frame.pack(side="left", padx=2)
    tb = NavigationToolbar2Tk(mpl_canvas, nav_frame)
    tb.config(bg="#080e1a")
    for child in tb.winfo_children():
        try:
            child.config(bg="#080e1a", fg=FG_MID, activebackground=CARD2, activeforeground=FG,
    ...
    _icon_btn(bar, "Save Chart", _save_png, bg="#1e3a8a",
              fg="#93c5fd", tip="Save chart as PNG")
...
def draw_plot(tab_frame, fig, tab_name, legend_builder=None, legend_kwargs=None):
    ...
    cw.config(bg="#0d1117", highlightthickness=0)
    cw.pack(fill="both", expand=True, padx=2, pady=2)
    _fill_toolbar(bar, mpl_canvas, tab_name)
    if legend_builder:
        legend_builder(leg_frame, **(legend_kwargs or {}))
...
def run_pipeline():
    ...
    try:
        # ── Step 1: User weights from the dynamic panel ────────────────────────
        ...
        # Ensure every feature column has a weight
        final_weights = {}
        for c in x_cols:
            final_weights[c] = weight_map.get(c, 1.0)
        total_fw = sum(final_weights.values())
        final_weights = {c: v/total_fw*100 for c, v in final_weights.items()}

        # ── Step 2: Weighted X + clustering ───────────────────────────────────
        ...
        a_labels, a_pct, a_idx, a_name, a_scores = \
            detect_anomalies(X_w, contamination=contam, method=method)
        ...
        active_df = wdf
        active_X = X_w
        active_xcols = use_cols
        active_anomaly_result = anomaly_result
        active_weight_result = {
            "weight_map": final_weights, "method_desc": method_desc}
        ...
        insights = generate_insights(wdf, use_cols, well_count, hx,
                                     anomaly_result, event_result,
                                     final_weights, method_desc, inertia)

        app.after(0, lambda: refresh_ui(
            wdf, X_w, use_cols, y_cols, hx, insights, well_count,
...
def refresh_ui(df, X, x_cols, y_cols, hx, insights, well_count,
    ...
    draw_plot(hidden_tab,
              plot_hidden_patterns(
                  X, anomaly_result["labels"], x_cols, anomaly_result["detector_name"]),
              "hidden_patterns",
...
def update_event_panel(event_result):
    if not event_result["has_events"]:
        ev_status_var.set("No event column in dataset")
        ev_status_lbl.config(fg=WARN)
        for v in ev_count_vars.values():
            v.set("N/A")
        ev_active_var.set("N/A")
        ev_inactive_var.set("N/A")
        ev_abnormal_var.set("N/A")
        ev_op_anom_var.set("N/A")
        return
    ...
    for i, k in enumerate(keys):
        if i < len(top):
            label, cnt = top[i]
            ev_count_vars[k].set(f"{cnt:,}")
            ev_count_lbls[k].config(text=label[:24])
        else:
            ev_count_vars[k].set("—")
            ev_count_lbls[k].config(text="—")
    ...
    ev_op_anom_var.set(
        f"{len(event_result.get('operational_anomalies',[])):,}")
        ...
        if ext == ".xlsx":
            with pd.ExcelWriter(path, engine="openpyxl") as writer:
                ...
                if active_weight_result:
                    ...
                    wdf_exp.to_excel(
                        writer, sheet_name="Parameter Weights", index=False)
            ...
            set_status(
                f"Exported (5 sheets): {os.path.basename(path)}", SUCCESS)
...
def generate_insights(df, x_cols, well_count, hx, anomaly_result,
    ...
    sev = ("LOW" if a_pct < 2 else "MODERATE" if a_pct <
           8 else "ELEVATED" if a_pct < 20 else "HIGH")
    ...
    lines = [
        ...
        "", "  PARAMETER IMPORTANCE (user-assigned from dataset columns)",
        "  ──────────────────────────────────────────",
    ...
    if weight_map:
        ...
        for param, wt in sorted(weight_map.items(), key=lambda x: -x[1]):
            star = " ★" if wt == max_w else ""
            bar = "█"*max(1, int(wt/2))
            lines.append(f"  {param:<30} {bar:<25} {wt:.1f}%{star}")
    else:
        lines.append("  (equal weights)")
...
app.title("CBM AI Analytics Platform  v5.0")
...
brand = tk.Frame(top_bar, bg="#040810")
...
tk.Label(brand, text="Coalbed Methane Analytics Platform  v5.0",
         bg="#040810", fg=FG_DIM, font=FONT_SM).pack(side="left")
tk.Label(top_bar, text="Dynamic weights from dataset  ·  Any file format  ·  Event analysis",
...
# File format note
tk.Label(ds_body,
         text="Accepts: CSV, TSV, XLSX, XLS, XLSB, XLSM,\n"
              "ODS, JSON, Parquet, Feather, HDF5, Pickle\n"
              "Any unknown format is tried automatically.",
...
make_btn(ds_body, "  Upload Data File (any format)", upload_dataset,
         color="#0e7490", fg_col="#e0f7fa", tip="Load any data file — CSV, Excel, JSON, etc.")

info_row = tk.Frame(ds_body, bg=CARD)
info_row.pack(fill="x", pady=(6, 0))
rows_var = tk.StringVar(value="—")
cols_var = tk.StringVar(value="—")
num_cols_var = tk.StringVar(value="—")
for title, var, col in [("Total Wells", rows_var, COL_DATASET), ("Columns", cols_var, FG_DIM), ("Numeric", num_cols_var, COL_WEIGHTS)]:
    ...
    tk.Label(cf, textvariable=var, bg=CARD, fg=col, font=(
        "Courier New", 13, "bold")).pack(anchor="w")
...
# ── ③ PARAMETER WEIGHTAGE (dynamic — rebuilt on dataset load) ──────────────────
wt_outer = make_section_card(
    sidebar, "③ PARAMETER WEIGHTAGE  (from your dataset)", COL_WEIGHTS)
...
# Scrollable inner frame for weight rows
wt_canvas_outer = tk.Frame(wt_outer, bg=CARD, height=180)
wt_canvas_outer.pack(fill="x")
wt_canvas_outer.pack_propagate(False)
...
wt_canvas.pack(side="left", fill="both", expand=True)
...
weight_body.bind("<Configure>", lambda e: wt_canvas.configure(
    scrollregion=wt_canvas.bbox("all")))
wt_canvas.bind("<Configure>", _wt_resize)

# Placeholder text until dataset loads
...
for label, val in [("Isolation Forest  (faster, tree-based)", "iforest"),
    ...
    tk.Radiobutton(mrow, text=label, variable=anomaly_method_var, value=val,
                   ...
                   activeforeground=COL_ANOMALY, font=FONT_SM, wraplength=390).pack(anchor="w", pady=1)
tk.Label(anom_body, text="Contamination  (expected anomaly %)",
         bg=CARD, fg=FG_DIM, font=FONT_SM).pack(anchor="w")
anomaly_contam_var = tk.DoubleVar(value=5.0)
crow = tk.Frame(anom_body, bg=CARD)
...
tk.Label(crow, text="%   (typical CBM: 3–10%)", bg=CARD,
         fg=FG_DIM, font=FONT_XS).pack(side="left", padx=6)

# ── ⑤ FEATURE SELECTION ────────────────────────────────────────────────────────
feat_body = make_section_card(
    sidebar, "⑤ FEATURE SELECTION  (X axes for charts)", COL_FEATURES)
tk.Label(feat_body, text="Ctrl+click for multi-select.  Leave blank = all numeric used.",
         bg=CARD, fg=FG_DIM, font=FONT_XS, wraplength=390, justify="left").pack(anchor="w")
...
make_btn(run_body, "  Run AI Analysis", start_pipeline,
         color="#4f46e5", tip="Cluster + anomaly + event analysis with your weights")
if HAVE_TTKBS:
    progress_bar = ttk.Progressbar(
        run_body, mode="determinate", bootstyle="info-striped", length=390)
else:
...
ev_tile_row = tk.Frame(ev_body, bg=CARD)
ev_tile_row.pack(fill="x", pady=(0, 6))
...
for title, var, col in [("Active Wells", ev_active_var, COL_EVENTS), ("Inactive", ev_inactive_var, COL_ANOMALY),
...
ev_count_vars = {}
ev_count_lbls = {}
ev_detail_frame = tk.Frame(ev_body, bg=CARD)
ev_detail_frame.pack(fill="x")
