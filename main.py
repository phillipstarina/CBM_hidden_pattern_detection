...
─────────────────────────────────────────────
KEY CHANGES in v5.0:
  • Microsoft Access 2007 (.mdb / .accdb) support via pandas_access
  • Parameter weightage section is DYNAMIC
  • Robust file loader — never crashes
  • Single Export button (Excel multi-sheet)
  • Event detection panel
  • Clean sidebar — only bold headings, no description text
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# ROBUST FILE LOADER  — supports Access 2007 (.mdb/.accdb) + all common formats
# ══════════════════════════════════════════════════════════════════════════════
def _load_access_file(filepath):
    """Load a Microsoft Access .mdb or .accdb file via pandas_access."""
    try:
        import pandas_access as mdb
        tables = mdb.list_tables(filepath)
        if not tables:
            raise RuntimeError("No tables found in Access database.")
        # Pick largest table by default
        best_table = tables[0]
        best_len   = 0
        for t in tables:
            try:
                df = mdb.read_table(filepath, t)
                if len(df) > best_len:
                    best_len   = len(df)
                    best_table = t
            except Exception:
                pass
        df = mdb.read_table(filepath, best_table)
        return df, f"Access table: '{best_table}' (of {len(tables)} tables)"
    except ImportError:
        raise RuntimeError(
            "pandas_access is not installed.\n"
            "Run:  pip install pandas-access\n"
            "Note: mdbtools must also be installed on your system."
        )
...
def load_any_file(filepath):
    ...
    # ── Microsoft Access ──────────────────────────────────────────────────────
    if ext in ("mdb", "accdb"):
        df, note = _load_access_file(filepath)
        if df is not None and len(df.columns) > 0:
            return df
        raise RuntimeError(f"Access file loaded but returned no data.  ({note})")
    ...
    raise RuntimeError(
        ...
        f"Supported: CSV, TSV, XLSX, XLS, XLSB, XLSM, ODS, JSON, Parquet, Feather,\n"
        f"           HDF5, Pickle, Microsoft Access (.mdb / .accdb).\n"
...
def make_section_card(parent, title, accent_color, **pack_kw):
    """Section card with ONLY a bold heading — no description text."""
...
def _mini_bar(parent, label, frac, col, label_width=8):
    ...
    tk.Label(row, text=label[:label_width], bg=CARD, fg=FG_DIM, font=FONT_XS,
...
def upload_dataset():
    ...
    def _post_load(df, filepath, num_cols):
        ...
        fmt = "Access DB" if ext in (".mdb",".accdb") else os.path.basename(filepath)
        set_status(f"✔  Loaded: {fmt}  ({len(df):,} rows × {len(df.columns)} cols)", SUCCESS)
...
def build_weight_legend(parent, weight_map, method_desc):
    ...
    tk.Label(parent, text="★ = most influential", bg=CARD, fg=FG_DIM, font=FONT_XS,
...
tk.Label(top_bar,
         text="Access 2007 (.accdb/.mdb)  ·  Dynamic weights  ·  Event analysis",
...
# ══════════════════════════════════════════════════════════════════════════════
# LEFT SIDEBAR  — only bold headings, no description text in cards
# ══════════════════════════════════════════════════════════════════════════════
...
make_btn(ds_body, "  Upload Data File", upload_dataset,
         color="#0e7490", fg_col="#e0f7fa", tip="Load any data file — Access, CSV, Excel, JSON…")
...
for title,var,col in [("Total Wells",rows_var,COL_DATASET),
...
make_btn(run_body, "  Run AI Analysis", start_pipeline,
         color="#4f46e5", tip="Cluster + anomaly + event analysis")
if HAVE_TTKBS:
...
tk.Label(rp_hdr,
         text="Access 2007  ·  Dynamic parameter weights  ·  Event analysis",
...
explain_text.insert("end",
    ...
    "      Accepts: Access 2007 (.accdb / .mdb),\n"
    "      CSV, Excel, JSON, Parquet, HDF5, Pickle\n\n"
    "  ②  Set number of clusters\n\n"
    "  ③  Adjust parameter weights for your columns\n"
    "      (spinboxes auto-populate after upload)\n\n"
    ...
    "  The largest table is loaded automatically.\n"
    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
)
explain_text.config(state="disabled")
