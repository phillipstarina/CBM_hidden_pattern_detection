...
def plot_production(df, ycols, xcols, hex_colors):
    ...
    if y_col is None:
        ax.text(0.2, 0.5, "Select a Y feature for production curve",
                transform=ax.transAxes, color=FG_DIM, fontsize=10); return fig
    for cl in sorted(df["cluster"].unique()):
...
def plot_weight_chart(weight_map, method_desc):
    ...
    ax_note.text(0.01, 0.8, f"Method: {wrapped}", transform=ax_note.transAxes, color=FG_DIM, fontsize=7.5, va="top")
...
def build_cluster_legend(parent, df, hex_colors):
    ...
    tk.Label(parent, text="CLUSTER LEGEND", bg=CARD, fg=COL_CLUSTER, font=FONT_H3, justify="center").pack(pady=(12,4), padx=8)
    _divider(parent, COL_CLUSTER)
    clusters=sorted(df["cluster"].unique()); total=len(df)
    for cl in clusters:
        cnt=int((df["cluster"]==cl).sum()); pct=cnt/total*100; col=hex_colors[int(cl)%len(hex_colors)]
        row=tk.Frame(parent, bg=CARD2, padx=6, pady=4, highlightbackground=col, highlightthickness=1)
        ...
        tk.Label(txt, text=f"{cnt:,} wells  ({pct:.1f}%)", bg=CARD2, fg=FG_MID, font=FONT_XS, anchor="w").pack(anchor="w")
    _divider(parent)
    for cl in clusters:
...
def build_anomaly_legend(parent, n_normal, n_anomaly):
    for w in parent.winfo_children(): w.destroy()
    tk.Label(parent, text="PATTERN LEGEND", bg=CARD, fg=ANOMALY_C, font=FONT_H3, justify="center").pack(pady=(12,4), padx=8)
    ...
    for label, count, col, icon in [("Normal Wells",n_normal,NORMAL_C,"●"),("Anomaly Wells",n_anomaly,ANOMALY_C,"◆")]:
        ...
        tk.Label(txt, text=f"{count:,} wells  ({pct:.1f}%)", bg=CARD2, fg=FG_MID, font=FONT_XS, anchor="w").pack(anchor="w")
    if total:
        _divider(parent)
...
def build_weight_legend(parent, weight_map, method_desc):
    for w in parent.winfo_children(): w.destroy()
    tk.Label(parent, text="PARAM WEIGHTS", bg=CARD, fg=COL_WEIGHTS, font=FONT_H3, justify="center").pack(pady=(10,2), padx=8)
    tk.Label(parent, text="★ = most influential", bg=CARD, fg=FG_DIM, font=FONT_XS, justify="center").pack(pady=(0,4))
    ...
    for i,(param,wt) in enumerate(sorted_wm):
        ...
        tk.Label(txt, text=f"{wt:.1f}%", bg=CARD2 if is_max else CARD, fg=FG_MID, font=FONT_XS, anchor="w").pack(anchor="w")
...
def build_event_legend(parent, event_result):
    for w in parent.winfo_children(): w.destroy()
    tk.Label(parent, text="EVENT LEGEND", bg=CARD, fg=COL_EVENTS, font=FONT_H3, justify="center").pack(pady=(10,2), padx=8)
    _divider(parent, COL_EVENTS)
    if not event_result["has_events"]:
        tk.Label(parent, text="No event column\nin dataset", bg=CARD, fg=WARN, font=FONT_SM, justify="center").pack(pady=8); return
    ...
    for label,cnt in sorted(counts.items(), key=lambda x:-x[1]):
        col=_c(label)
        row=tk.Frame(parent, bg=CARD2, padx=6, pady=3, highlightbackground=BORDER, highlightthickness=1)
        row.pack(fill="x", padx=8, pady=2)
        tk.Canvas(row, bg=col, width=18, height=18, highlightthickness=0).pack(side="left", padx=(0,6))
        txt=tk.Frame(row, bg=CARD2); txt.pack(side="left", fill="x", expand=True)
        tk.Label(txt, text=label[:20], bg=CARD2, fg=col, font=FONT_SM, anchor="w").pack(anchor="w")
        tk.Label(txt, text=f"{cnt:,}  ({cnt/total*100:.1f}%)", bg=CARD2, fg=FG_MID, font=FONT_XS, anchor="w").pack(anchor="w")
...
def resolve_features():
    if raw_data is None: return [], None, 0, "No dataset loaded"
...
def export_results():
    ...
    try:
        ...
        if ext==".xlsx":
            ...
            set_status(f"Exported (5 sheets): {os.path.basename(path)}", SUCCESS)
...
tk.Label(anom_body, text="Contamination  (expected anomaly %)", bg=CARD, fg=FG_DIM, font=FONT_SM).pack(anchor="w")
...
for stat_row_defs in [stat_defs_r1, stat_defs_r2]:
    ...
    for title,val,col in stat_row_defs:
        sf=tk.Frame(row, bg=CARD, padx=10, pady=7, highlightbackground=col, highlightthickness=1)
